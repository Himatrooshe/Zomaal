...
@Injectable()
export class YouCanTokenEncryptionService {
  ...
  decrypt(value: string, context: string): string {
    ...
    if (version !== 'v1' || !iv || !tag || !encrypted) {
      throw new ServiceUnavailableException(
        'Stored YouCan credentials are invalid',
      );
    }
    ...
    try {
      ...
      throw new ServiceUnavailableException(
        'Unable to decrypt YouCan credentials',
      );
    }
  }
  ...
  private getKey(): Buffer {
    const configured = this.configService
      .get<string>('YOUCAN_TOKEN_ENCRYPTION_KEY')
      ?.trim();
