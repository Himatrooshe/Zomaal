...
@Injectable()
export class QuickLivraisonOverviewService {
  constructor(private readonly prisma: PrismaService) {}

  async getOverview(userId: string, query: QuickLivraisonOverviewQueryDto) {
    ...
    const [
      ...
      this.prisma.quickLivraisonConnection.findUnique({
        where: { userId },
...
function performanceTrend(from: Date, days: number, rows: TrendShipment[]) {
  ...
  for (const shipment of rows) {
    ...
    if (delivered) {
      const point = points.get(utcDateKey(delivered.eventAt));
      if (point) point.delivered += 1;
    }
    if (returned) {
      const point = points.get(utcDateKey(returned.eventAt));
