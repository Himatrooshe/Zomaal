...
const STATUS_MAP: Record<string, ShippingShipmentStatus> = {
  ...
  EN_ATTENTE: ShippingShipmentStatus.PENDING,
  ...
  RAMASSAGE_PROGRAMME: ShippingShipmentStatus.PICKUP_PENDING,
...
export function normalizeOzoneExpressStatus(
  value: string | null | undefined,
): ShippingShipmentStatus {
  const key = normalizeOzoneExpressStatusCode(value);
  ...
  if (key.includes('DISTRIB') || key.includes('COURS_DE_LIVRAISON')) {
    return ShippingShipmentStatus.OUT_FOR_DELIVERY;
  }
...
export function normalizeOzoneExpressStatusCode(
  value: string | null | undefined,
): string | null {
...
export function ozoneFeeForStatus(
  ...
  if (status === ShippingShipmentStatus.DELIVERED) return prices.deliveredPrice;
  if (status === ShippingShipmentStatus.REFUSED) return prices.refusedPrice;
  if (
    (
      [
      ...
      ] as ShippingShipmentStatus[]
    ).includes(status)
  ) {
    return prices.returnedPrice;
  }
  return null;
}
