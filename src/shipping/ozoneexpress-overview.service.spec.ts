...
describe('OzoneExpressOverviewService', () => {
  const groupBy = jest.fn();
  const findMany = jest.fn();
  const findFirst = jest.fn();
  const connectionFindUnique = jest.fn();
  const service = new OzoneExpressOverviewService({
    ozoneExpressShipment: { groupBy, findMany, findFirst },
  ...
  it('returns KPIs, top cities, and synchronization health', async () => {
    groupBy
