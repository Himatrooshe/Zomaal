...
@Injectable()
export class OzoneExpressOverviewService {
  constructor(private readonly prisma: PrismaService) {}

  async getOverview(userId: string, query: OzoneExpressOverviewQueryDto) {
    const days = query.days ?? 7;
    const now = new Date();
    const from = startOfUtcDay(
      new Date(now.getTime() - (days - 1) * 86_400_000),
    );
    const [groups, deliveredRows, trendRows, cityGroups, latest, connection] =
      await Promise.all([
        ...
        this.prisma.ozoneExpressConnection.findUnique({
          where: { userId },
    ...
    return {
      ...
      performance: trend(from, days, trendRows),
...
function trend(from: Date, days: number, rows: TrendShipment[]) {
  const points = new Map<
    string,
    { date: string; shipmentCount: number; delivered: number; returned: number }
  >();
  for (let index = 0; index < days; index += 1) {
    const key = dateKey(new Date(from.getTime() + index * 86_400_000));
    points.set(key, { date: key, shipmentCount: 0, delivered: 0, returned: 0 });
  }
  for (const shipment of rows) {
    const created = points.get(
      dateKey(shipment.providerCreatedAt ?? shipment.createdAt),
    );
    ...
    if (delivered) {
      const point = points.get(dateKey(delivered.eventAt));
      if (point) point.delivered += 1;
    }
    if (returned) {
      const point = points.get(dateKey(returned.eventAt));
      if (point) point.returned += 1;
    }
  }
